...
def test_paged_flash_decoding_small_head_size():
    ...
    # CHECK:                   %[[COND_SPLAT:.*]] = vector.broadcast %{{.*}} : i1 to vector<1xi1>

    # CHECK:                   scf.for

    # v_cache reads use per-element masked loads with head_size bounds check.
    # CHECK:                     amdgpu.lds_barrier
    # CHECK:                     vector.maskedload %{{.*}}[%{{.*}}], %[[COND_SPLAT]]
    # CHECK:                     vector.maskedload %{{.*}}[%{{.*}}], %[[COND_SPLAT]]
    # CHECK:                     vector.maskedload %{{.*}}[%{{.*}}], %[[COND_SPLAT]]
    # CHECK:                     vector.maskedload %{{.*}}[%{{.*}}], %[[COND_SPLAT]]
