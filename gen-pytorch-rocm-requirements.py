...
# ---------------------------------------------------------------------------
# URL probing -- verify a matching torch version exists
# ---------------------------------------------------------------------------
...
def _parse_spec_bounds(spec: str) -> tuple[tuple[int, ...] | None, tuple[int, ...] | None]:
...
def _has_matching_torch(flag: str, url: str, torch_spec: str, timeout: float = 15) -> bool:
    ...
    try:
        html = urlopen(Request(page_url), timeout=timeout).read().decode(
